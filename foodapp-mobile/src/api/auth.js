...
export const loginWithFirebase = async (firebaseToken) => {
  const res = await axios.post('api/auth/firebase', { token: firebaseToken });
  return handleTokenResponse(res);
};

export const requestForgotPassword = async (email) => {
  const res = await axios.post('api/auth/forgot-password/request', { email });
  return res.data;
};

export const resetPasswordEmail = async (email, code, newPassword) => {
  const res = await axios.post('api/auth/forgot-password/reset', { email, code, newPassword });
  return res.data;
};

export const resetPasswordPhone = async (firebaseToken, newPassword) => {
  const res = await axios.post('api/auth/forgot-password/phone', { token: firebaseToken, newPassword });
  return res.data;
};
